  ...
  Future<void> _processPayment() async {
    ...
    try {
      for (final item in widget.items) {
        final prod = await Supabase.instance.client
            .from('products')
            .select('stock, name')
    ...
    try {
      ...
      final inserted = await Supabase.instance.client
          ...
          .maybeSingle();
      if (inserted == null) {
        throw Exception('Order insert failed');
      }

      // Decrement stock for each ordered item, and mark is_sold when stock hits 0
      for (final item in widget.items) {
        final updated = await Supabase.instance.client.rpc('decrement_product_stock', params: {
          'p_product_id': item.productId,
          'p_quantity': item.quantity,
        });
        // Fallback if RPC not available
        if (updated == null) {
          final current = await Supabase.instance.client.from('products').select('stock').eq('id', item.productId).maybeSingle();
          final currentStock = (current?['stock'] ?? 0) as int;
          final newStock = currentStock - item.quantity;
          await Supabase.instance.client
              .from('products')
              .update({'stock': newStock < 0 ? 0 : newStock, 'is_sold': (newStock <= 0)})
              .eq('id', item.productId);
        }
      }
      ...
      if (mounted) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => OrderDetailScreen(order: PurchaseOrder.fromMap(inserted)),
          ),
        );
      }
    } catch (e) {
