...
class _AdminOrdersScreenState extends State<AdminOrdersScreen> {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (status.isGranted) {
          ...
          final file = File('${directory.path}/orders_report_${DateTime.now().toIso8601String().replaceAll(':', '-')}.pdf');
  ...
  void _exportSelectedToPDF() async {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (status.isGranted) {
          final directory = await getApplicationDocumentsDirectory();
          final file = File('${directory.path}/selected_orders_report_${DateTime.now().toIso8601String().replaceAll(':', '-')}.pdf');
          await file.writeAsBytes(await pdf.save());
          await OpenFilex.open(file.path);
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _saveAndOpenFile({required List<int> bytes, required String suggestedName}) async {
    try {
      final status = await Permission.storage.request();
      if (!status.isGranted) {
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Text('Total: ${order['total'] ?? ''}', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green[800]),),
                    ...
                    Wrap( // Changed from Row to Wrap to handle overflow
                      ...
                      children: [
                        ...
                        Builder(
                          builder: (context) {
                            final status = (order['status'] ?? '').toString().toLowerCase();
                            // Hide pencil for rejected/cancelled/delivered
                            if (status == 'rejected' || status == 'cancelled' || status == 'delivered') {
                              return SizedBox.shrink();
                            }
                            // Build allowed transitions by current status
